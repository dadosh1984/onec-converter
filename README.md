...
**SQL-источники: ограничения (честный контракт, Фаза 36/43):** модуль
`sql_source` — адаптер поверх драйвера, а НЕ полноценный парсер серверной
ИБ: имена объектов = имена таблиц (без разбора `v8_metadata`), типы колонок
пустые, приоритет операторов и экранирование идентификаторов — по правилам
конкретной СУБД. Подключение с таймаутом (`connect_timeout`, по умолчанию
10 с), выборка строк потоковая (`fetchmany`). Для production-переноса
конфигурации используйте файловый источник `1Cv8.1CD`; SQL-источник — для
сырых данных и экспериментов (см. TODO в докстроке `sql_source.py`).

**Feature matrix (Фаза 46):**

| Возможность | 7.7 | Файловая 8.x | SQL 8.x (postgres/mssql) |
|---|---|---|---|
| Чтение данных (extract) | ✅ (v77) | ✅ | ✅ (адаптер) |
| Структура из конфигурации | ⚠️ внешние метаданные | ✅ `1Cv8.1CD` | ⚠️ имена таблиц |
| Прямая запись (load) | ❌ | ✅ (8.x) | ❌ |
| Проверка целостности | — | ✅ `verify` | — |
| Аудит-журнал | ✅ общий | ✅ общий | ✅ общий |
...
**Tamper-evident audit log (комплаенс, Фаза 42/46):** цепочка SHA-256
`hash = H(prev_hash || запись)` — изменение/удаление любой записи ломает
все последующие хеши и детектируется `onec-converter audit-verify
--audit-file audit.jsonl [--cross-files]` (проверка границ архивов `.1/…`).
Первая запись обязана иметь пустой `prev_hash`; ротация пишет маркер
`{"marker":"rotated","prev_hash":…}`. Пример для внедрения:
`load --audit-file`, затем `audit-verify` в CI/по расписанию (см.
`docs/recipes/полный-цикл-clone-load-verify-audit.md`).
...
## Обратный контроль через xlsx-мост (bridge-verify)

`bridge-export` выгружает справочник/регистр источника в xlsx-мост (аналог
epf-макета), `bridge-import` загружает его в КОПИЮ приёмника
(find-or-create). `bridge-verify` — обратный контроль: данные выгружаются
обратно из копии приёмника и построчно сравниваются с исходным мостом по
всем колонкам (не только по полю поиска).

```
onec-converter bridge-verify --input bridge.xlsx --target-dir КОПИЯ_приёмника \
    --type Справочник.Банки [--key Код,Наименование] [--ignore-cols _VERSION,_MARKED]
```

- значения нормализуются перед сравнением: `1 == 1.0`, даты — ISO,
  строки — strip, пустые/None — эквивалентны (не даёт ложных расхождений);
- `--key` — составной ключ сверки (список колонок через запятую,
  по умолчанию — первое поле поиска);
- `--ignore-cols` — служебные колонки (`_VERSION`, `_MARKED` и пр.),
  исключаемые из сравнения;
- расхождения уровня `different` содержат `cols` — какие именно колонки
...
## bridge-migrate: документы и табличные части

`bridge-migrate` переносит пользовательские данные источник -> копия
приёмника по разделам (отдельный xlsx-мост на раздел) с обратным тестом.

- **Справочник** — find-or-create по `Код`/`Наименование`.
- **Регистр сведений** — записи по измерениям.
- **Документ** — шапка (поиск по `Номер`).
- **Табличная часть** — каждый раздел `Документ.Х.ТЧ.<таблица>`
  мостом режима 1; строки ТЧ дописываются к документу-владельцу.
- **Пилотный прогон**: первыми `--pilot-rows` (3) строками — при
  расхождении полная загрузка раздела не выполняется.
- **Фаза сравнения метаданных**: перед переносом структура user-объектов
