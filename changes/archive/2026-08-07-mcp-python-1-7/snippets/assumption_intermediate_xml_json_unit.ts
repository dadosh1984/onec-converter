// GREEN: intermediate — сериализация объектов в XML/JSON (атрибуты, ссылки как естественные ключи)
import * as fs from 'node:fs';
import * as path from 'node:path';

export function assumption_intermediate_xml_json_unit() {
  const files: Record<string, string> = {
    'src/onec_converter/intermediate.py': `"""Промежуточный формат переноса: объекты в XML/JSON.
...
`,
    'tests/test_intermediate.py': `"""Unit-тесты промежуточного формата."""
from onec_converter.intermediate import (make_object, to_json, from_json, to_xml,
                                        save_json_batch, load_json_batch)
