// GREEN: mapping — резолвер ссылок по естественным ключам + коллизии/отсутствующие ссылки
import * as fs from 'node:fs';
import * as path from 'node:path';

export function assumption_mapping_unit() {
  const files: Record<string, string> = {
    'src/onec_converter/resolver.py': `"""Резолвер ссылок по естественным ключам.
...
from .intermediate import OBJ_TYPE, OBJ_KEY, OBJ_ID
...
class RefResolver:
    ...
    def resolve(self, obj_type: str, key: tuple[str, ...], source_id: str) -> str | None:
        ...
        return ids[0]
`,
    'tests/test_mapping_resolver.py': `"""Unit-тесты резолвера ссылок."""
