// GREEN: verify — сверка полноты «источник ↔ приёмник» (количество, контрольные суммы, отчёт)
import * as fs from 'node:fs';
import * as path from 'node:path';

export function assumption_verify() {
  const files: Record<string, string> = {
    'src/onec_converter/verify.py': `"""Финальная верификация переноса: сверка «источник ↔ приёмник».
...
from .intermediate import OBJ_TYPE, OBJ_KEY, OBJ_ATTRS
...
def verify(source_objects: Iterable[dict[str, Any]],
    ...
    return report
`,
    'tests/test_verify.py': `"""Unit-тесты verify."""
from onec_converter.intermediate import make_object
from onec_converter.verify import verify, checksum
