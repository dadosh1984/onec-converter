// GREEN: inspect_target — структура приёмника 8.3 (1Cv8.1CD напрямую + /metadata), правило 1→1
import * as fs from 'node:fs';
import * as path from 'node:path';

export function assumption_inspect_target_8_3_1cv8_1cd() {
  const files: Record<string, string> = {
    'src/onec_converter/inspect_target.py': `"""Структура приёмника 8.3 и привязка пары источник→приёмник (правило 1→1).
...
def inspect_target_from_1cd(target_1cd: str | Path) -> TargetMetadata:
    ...
    return read_metadata(target_1cd)
`,
    'tests/test_inspect_target.py': `"""Unit-тесты inspect_target и правила 1→1."""
import pytest

from onec_converter.inspect_target import (ProjectBinding, ProjectError,
                                           inspect_target_from_http)
