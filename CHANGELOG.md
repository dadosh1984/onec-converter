...
- `bridge-migrate` — перенос пользовательских данных расширен:
  - **Документы**: выгрузка шапки документа в мост (поиск по `Номер`),
    импорт find-or-create как у справочников;
  - **Табличные части**: каждый раздел `Документ.Х.ТЧ.<таблица>` — отдельный
    мост (режим 1); строки ТЧ дописываются к найденному/созданному
    документу-владельцу; план включает шапку и каждую ТЧ;
  - **Фаза сравнения метаданных** (шаги 5-6): структура user-объектов
    источника сверяется с приёмником (состав реквизитов, наличие таблиц и
    ТЧ-таблиц); разделы с расхождением исключаются из плана и попадают
    в отчёт `meta_conflicts`;
  - CLI `--meta-conflicts` — выводит только отчёт о расхождениях
    структуры, без запуска переноса.
- Надёжность экспорта: санитизация строк для xlsx (control-символы,
  суррогаты, бинарные NT-поля → `None`), чтобы реальные базы (1C_8.1)
  экспортировались без `IllegalCharacterError` (БанковскаяВыписка: 2286
  строк, ТЧ: 14462).

## 0.46.0 (2026-08)

### bridge-verify: надёжность обратного контроля; fetch-config: англ. теги

- `bridge-verify` — обратный контроль через xlsx-мост стал надёжнее:
  - нормализация значений при сравнении (числа `1 == 1.0`, даты ISO,
    строки strip, пустые/None эквивалентны) — нет ложных расхождений;
  - diff на уровне полей: для `different` видно, какие колонки и значения
    различаются (in vs out);
  - составной ключ сверки `--key Код,Наименование`;
  - `--ignore-cols` — служебные колонки (`_VERSION`, `_MARKED`),
    исключаемые из сравнения.
- `fetch-config` — поддержка английских тегов MDClasses XML-выгрузки
  (`Catalog`, `Document`, `Constant`, ...): русские теги по-прежнему
  работают; проверено на выгрузках 8.1 (787 объектов) и 8.3 (7891).

## 0.45.0 (2026-08)

### xlsx-мост «Выгрузка и загрузка XML»: CLI + enum-поддержка

- Команды CLI: `bridge-export <объект> --out bridge.xlsx` (справочник/регистр
  источника → xlsx-мост формата .epf), `bridge-import <мост> --target Копия.1CD`
  (find-or-create по полям поиска), `bridge-verify` (обратный контроль: выгрузить
  из копии приёмника в мост и сверить).
- **Перечисления**: `Перечисление.X` разрешается по синониму → `_IDRREF`
  приёмника. Имена/синонимы значений извлекаются из CONFIG приёмника
  (`enum_resolver.extract_enum_values`, порядок `_ENUMORDER`), GUID-строка
  конвертируется в 16 байт `_IDRREF` (layout `p3+p4+p2+p1+p0`). Проверено на
  реальной базе 1C_8.1 — позиционное соответствие 35/35.
- **Расширения lookup**: `СвязьПоВладельцу` (поиск в подчинённых/иерархических
  справочниках с фильтром по владельцу), документы «Номер от Дата»,
  `resolve(owner=...)` по `_OWNERIDRREF`.
- **Опции записи** `write_8x.update_catalog`: обновление существующих записей по
  полям поиска вместо дублирования; корректный новый `_IDRREF` без коллизий.
- `typify` — типизация ячеек (число/дата/булево/строка) при экспорте.
- **Значения по умолчанию**: `Сегодня()`/`ТекущаяДата()` -> дата/время,
  `ПустоеЗначение()`/`НовыйОбъект()` -> поле не трогать (аналог epf).
- Ворота green: pytest 605, ruff, mypy strict, vitest 355, MCP-conformance,
  benchmark. Открытый остаток: мета `read_metadata` не несёт полное имя
  ссылочного типа для `bridge_verify` (сверка enum-колонок по синониму).

## 0.44.1 (2026-08)

### Багфикс прогресс-бара (вне TTY)
- В не-интерактивном выводе (redirect/MCP) прогресс печатался на каждый объект
  (условие `pct % 10 == 0` срабатывало при 0%). Теперь узлы выводятся только
  на старте (0%), каждые 10% и на финале — stderr не засоряется.

## 0.44.0 (2026-08)

### Визуальный прогресс переноса в терминале
- `migrate --direct` и `load_direct` теперь выводят в stderr визуальный
  progress-бар: процент, число перенесённых объектов (например
  `📚 62% (2643/4251) [_Reference98]`), а по завершении — сводку таблиц
  приёмника (`_Reference98: 2060 записей`). Вывод идёт в stderr, так что
  JSON-результат в stdout не ломается (машиночитаемость сохранена);
  в MCP это же видно в stderr-логах клиента.

### Багфикс: чтение дат-заглушек и сериализация BLOB в extract
- `decode_datetime` падала на невалидной дате-заглушке 1С (`0100-00-00`),
  ломая extract/migrate на реальных базах. Теперь такие даты возвращают
  `None` (как нулевые), а не бросают ValueError.
- `save_json_batch`/`save_json_stream` падали при записи BLOB-полей
  (`Object of type bytes is not JSON serializable`) — фото/двоичные данные
  в справочниках лишались экстракт. `bytes` теперь кодируются в base64.

## 0.43.2 (2026-08)

### Багфикс: плейбук/скилл ссылались на несуществующие MCP-тулы
- Плейбук (`PLAYBOOK`, `playbook()`), `docs/playbook.md` и скилл
  `onec-converter-migration` советовали агенту вызывать `step_init`,
  `step_extract`, `step_map`, `step_load`, `step_prevalidate`, `step_inspect_*`,
  `verify`, `transform`, `preview` — этим команды НЕ выставляются как MCP-тулы
  (это внутренние шаги тула `migrate()`). Агент получал «Unknown tool» и
  скилл не работал.
- Теперь плейбук и скилл ссылаются только на реальные 18 MCP-тулов
  (разведка → сравнение/маппинг → `migrate`/`load_direct` → сверка `guid_diff`);
  поле `next` каждого тула указывает на существующие команды.
- Скилл синхронизирован с глобальной копией `~/.pi/agent/skills/` (pi/агенты).
- Новый тест-защита `tests/test_mcp_tool_surface.py`: каждый тул из
  PLAYBOOK/playbook/docs/SKILL обязан существовать в реестре сервера
  (защита от регресса «Unknown tool»).
- Ворота green: pytest 557 / ruff (src+tests) / mypy (src+scripts) /
  vitest 355, E2E stdio (tools/list = 18 тулов).

## 0.43.1 (2026-08)

### Багфиксы версии/обновления (для всех агентов)
- SemVer-сравнение: если установлена версия новее PyPI (dev/editable),
  ложное уведомление об обновлении больше не показывается
  (0.42.0 из кеша не «новее» 0.43.1).
- Подавлен pydantic-WARNING MCP SDK в stderr при старте — баннер версии
  теперь первое и единственное сообщение (чистый вывод для pi/Claude,
  которые читают stderr процесса).

## 0.43.0 (2026-08)

### Версия и обновление MCP-сервера (для всех агентов)
- `serverInfo.version` при `initialize` теперь НАША версия релиза (0.43.0),
  а не версия MCP SDK — любой MCP-клиент (Claude/Cursor/pi/другие) видит
  версию инструмента по стандарту.
- Каждый ответ тула содержит `server_version` (и `update`, если на PyPI
  вышла более новая версия + инструкция по обновлению) — доходит до ЛЮБОГО
  агента через результат тула, независимо от того, показывает ли хост stderr.
- При старте сервера в stderr печатается баннер релиза и, если есть новая
  версия, уведомление об обновлении (`ONEC_NO_UPDATE_CHECK=1` — отключить
  проверку). Проверка PyPI кэшируется на сутки (.onec_cache/version-check.json)
  и не блокирует старт.
- Скилл `onec-converter-migration` — инструкции агенту по переносу 1С
  (устанавливается и для pi: ~/.pi/agent/skills/).
- Ворота green: pytest (+6), ruff, mypy (55), conformance, vitest (355).

## 0.42.0 (2026-08)

### Документация и продукт (Фаза 59)
- I1: README-матрица команд дополнена `migrate`, `wizard`, `mcp --stdio`;
  docs/commands-map.md (33) синхронизирован (контракт-тест в воротах).
- I2: docs/backlog.md — отдельный честный «бэклог после 0.42.0»: U57 двуязычный
  help, U58 smoke-PR, U59 плагины/хуки, U60 typed-конфиг, U61 ошибки с
  контекстом, U56 list-tables SQL, вариант B zero-setup.
- I3: docs/recipes/полный-цикл-migrate.md — сценарий сквозного переноса
  одной командой (с правилами, --direct, wizard).
- Ворота green (документные контракт-тесты зелёные).

## 0.41.0 (2026-08)

### Производительность и ворота (Фаза 58)
- H1: `gates.sh all` теперь включает benchmark (пороги скорости) — раньше
  он был отдельной целью.
- H2: контракт-тест: тулы CLI `mcp` == тулы mcp_server (18) — дрейф ловится.
- H4: зафиксирован тестом единый источник COVERAGE_MODULES (pyproject; 
  gates.sh читает оттуда, имена не захардкожены в скрипте).
- H5: hypothesis fuzz audit hash-цепочки — любая мутация записи
  обязательно разрывает verify (запись→чтение→сверка).
- D4: net-оп — `read_metadata` уже имеет mem-LRU (Фаза 49 U39) + дисковый
  кеш; ai-map/ai-explain переиспользуют его автоматически.
- Ворота green: pytest (+5, Фаза 58), ruff, mypy, benchmark ok, vitest.

## 0.40.0 (2026-08)

### Безопасность (Фаза 57)
- E3: секреты `--secret` (mint-token, load) читаются из env `ONEC_SECRET`
  или ввода без эха в истории — не светят в ps/history/log.
- E4: политика `snapshot`: гарантия отката/инспекции сохранена;
  пользователь может не создавать snapshot-копию через `--no-snapshot`
  (`snapshot=False`) — освобождение сотен МБ, если копия не нужна.
  Авто-удаление на успехе НЕ делал: это ломало бы документированный
  контракт отката и существующий тест (баланс «не сломать»).
- E1: пароль в DSN/URL SQL-источников маскируется (`secret_mask.mask_dsn`)
  в исключениях — проверено, доп. правок не требуется.
- A7: экранирование формул CSV в `audit --csv-out` (см. 0.37.0).
- Ворота green: pytest (+4, Фаза 57), ruff, mypy, conformance, vitest.

## 0.39.0 (2026-08)

### Функциональность (Фаза 56)
- C1: `mcp --stdio/--sse` — запуск MCP-сервера из CLI (раньше
  `python -m onec_converter.mcp_server` только импортировал модуль и
  завершался; точки входа `mcp_server.server_main()`).
- C4: `migrate` — сквозной перенос одной командой: extract → transform
  (правила TOON) → load (файл или --direct в копию 1CD); источники 7.7/8.x.
- G2: `wizard` — интерактивный мастер переноса (вопросы → команда migrate;
  --no-run печатает команду без выполнения).
- Команды CLI 31 -> 33; docs/commands-map.md обновлён.
- Ворота green: pytest (+6, 534), ruff, mypy, conformance, vitest.

## 0.38.0 (2026-08)

### Интерактивность и UX (Фаза 55)
- F1/F2: `--pretty` — ASCII-таблицы для inspect/query/stats/guid-diff;
  авто-включение в TTY, при `--no-pretty`/pipe остаётся JSON (без нарушения
  скриптовой интеграции).
- F3: `[done]`-заметки в stderr (только TTY) у extract/transform/load.
- G1/G3/F4: `--help` сгруппирован по категориям (Разведка/Перенос/Проверка/
  Отчёты и аудит/Служебные) — 31 команда читабельнее; корневые
  `--pretty`/`--no-pretty`.
- H-fix: `--help` больше не падает UnicodeEncodeError на cp1251-консолях
  Windows (UTF-8 reconfigure в main).
- `terminal.render_table` / `terminal.is_tty` — человек-читаемые таблицы без
  новых зависимостей.
- Ворота green: pytest (+9 Фаза 55), ruff, mypy, conformance, vitest.

## 0.37.0 (2026-08)

### Дефекты и чистка (Фаза 54, раунд аудита 6)
- A1: убран мёртвый код в `ai_skills.auto_map_schemas` (неиспользуемая
  `src_attrs`/`del`).
- B5: единый декодер `_table_row_to_rec` — убрано дублирование чтения
  таблицы в `dump-records` и `export-xlsx`.
- B6: константа `DEFAULT_SOURCE_ENCODING` (cp866) вместо хардкода в 3 местах.
- A2: `config.load` — strip() всех строковых значений конфига (onec.toml).
- A5: `load` без `--target`/`--http`/`--direct` — понятная ошибка вместо
  молчаливого `./load.json`.
- A7: `audit --csv-out` — экранирование формул Excel (`=`, `+`, `-`, `@`)
  против формульной инъекции.
- A3/A8: зафиксирован контракт потоков (stdout=данные, stderr=метаданные)
  у `audit --json` и `sonar-report`.
- Тест: `test_cli_entrypoint` устойчив к cp1251-консолям Windows
  (PYTHONIOENCODING=utf-8) — раньше `--help` с «\u2194» ронял subprocess.
- Ворота: pytest (+5, Фаза 54), ruff, mypy — green.

## 0.36.0 (2026-08)

### Продукт и востребованность (Фаза 53, финал раунда 5)
- U11: `export-xlsx` — первые N строк таблицы 1CD в XLSX (openpyxl, лист-таблица).
- U12: `map --init` — шаблон правил TOON из метаданных (каждый объект ->
  пустой target, реквизиты-заглушки).
- U13: `doctor --fix` — создаёт кеш и выводит команды установки недостающих
  зависимостей.
- U15: `mcp` — список MCP-тулов сервера (имя + описание).
- U16: `stats` — сводка по ИБ: число таблиц/строк, объём, locale.
  Реестр CLI 28 -> 31 (export-xlsx, stats, mcp).
- U51: README — матрица команд по группам.
- U52: docs/format-8x.md — инженерная справка о формате 1Cv8.1CD.
- U53: docs/recipes/бухгалтерия-77-в-83.md — готовый сценарий миграции.
- U54: docs/recipes/облачные-среды.md — поддержка 1С:Фреш/аренды через
  HTTP-сервис (прямая запись к облачным базам неприменима).
- U55: pii-report (профиль RU/UZ) — уже покрывал gdpr-отчёты; нет-оп.
- U56: list-tables для SQL-источников — в бэклог (требует реальный драйвер).
- U62: schema_version TOON (mapping.SCHEMA_VERSION=1) — уже в валидации; нет-оп.
- Ворота: pytest (+12), conformance 5, ruff чисто, mypy (54), vitest.

## 0.35.0 (2026-08)

### Безопасность (Фаза 52, раунд 5)
- U8/U27: secret_mask.py — маскирование секретов DSN/URL (userinfo, key=
  token/password/client_secret); sql_source маскирует исключения подключения
  (пароль не утекает).
- U28: s3 assume_role (STS AssumeRole Signed POST, без boto3) — временные
  ключи для сервисных учёток.
- U29/U32: BSL лимит пакета (413, max 1000 объектов) + идемпотентность
  в пределах пакета по полю idem; клиентская идемпотентность сетевых
  ретраев — через replace=true (честно: Перем не хранит состояние между
  запросами HTTP-сервиса 1С).
- U30: JWT kid/ротация — mint_jwt(kid), verify_jwt_kid (kid выбирает секрет
  жёстко, без kid — все для совместимости), CLI mint-token --kid; BSL
  ПроверитьJWT по НаборСекретовJWT.
- U31: pre-commit секрет-сканер в .githooks/pre-commit (AKIA/ASIА/PRIVATE
  KEY/password/client_secret в диффе блокируют коммит).
- U33: notify ретрай 5xx (транзиентные шлюзы) + сетевые; 4xx не ретраятся.
- Ворота: pytest (+11), conformance 5, ruff чисто, mypy (54), check_bsl,
  vitest.
